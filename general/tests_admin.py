...
def adminviews_test(self, user=None, password=None):
    ...
    for id_ in dir(models_mod.models):
        ...
        if isinstance(model, ModelBase) and model._meta.app_label == pkg and \
                model in admin.site._registry:
            try:
                # Prevent error 405 if model_admin.has_add_permission
                # always return False
                if admin.site._registry[model].has_add_permission(
                    ...
                    self.assertFalse(
                        "this_is_the_login_form" in response.content,
                        "login requested for {}".format(str(model)))
                url = reverse("admin:%s_%s_changelist" %
                              (model._meta.app_label, model._meta.module_name))
                response = self.client.get(url, follow=True)
                self.failUnlessEqual(
                    response.status_code, 200, "{} != {} -> {}, url: {}".format(
                        response.status_code, 200, repr(model), url))
